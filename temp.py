...
for date in range(5, 36):  # Adjusted range to match the number of rows on the page
    ...
    # データの挿入
    sql_insert_data = '''
        INSERT INTO weather (day, hpa, temp_ave, temp_max, temp_min, humidity_ave, humidity_min, sun)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?);
    '''
...
for date in range(5, 19):  # Adjusted range to match the number of rows on the page
    data = {}
